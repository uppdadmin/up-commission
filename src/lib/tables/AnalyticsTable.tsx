...
const AnalyticsTable = () => {
  ...
  const overallStats = useMemo(() => {
    ...
    const authorizedServices = filteredServices.filter(s => s.include_in_total !== false);
    const pendingServices = filteredServices.filter(s => s.include_in_total === false);

    const totalAmount = filteredServices.reduce((sum, service) => sum + (service.price || 0), 0);
    const authorizedAmount = authorizedServices.reduce((sum, service) => sum + (service.price || 0), 0);
    const pendingAmount = pendingServices.reduce((sum, service) => sum + (service.price || 0), 0);
  ...
  const monthlyStats = useMemo((): MonthlyStats[] => {
    ...
    Object.values(grouped).forEach(stats => {
      stats.avgServiceValue = stats.serviceCount > 0 ? stats.totalAmount / stats.serviceCount : 0;
    });
  ...
  const generatePDFReport = async () => {
    ...
    try {
      ...
      const reportContent = `
        ...
        <html>
        <head>
          <title>Relatório de Analíticas - ${isAdmin ? 'Admin - Todos os Serviços' : user?.firstName || 'Usuário'}</title>
          <style>
        ...
        <body>
          <div class="header">
            <h1>Relatório de Analíticas ${isAdmin ? '(Admin - Todos os Serviços)' : ''}</h1>
            <p>Usuário: ${user?.firstName || 'N/A'} ${user?.lastName || ''}</p>
            <p>Período: ${selectedPeriod === 'all' ? 'Todos os registros' : `Últimos ${selectedPeriod.replace('months', ' meses')}`}</p>
          ...
          <div class="stats-grid">
            ...
            <div class="stat-card">
              <div class="stat-value currency">${formatCurrency(overallStats.totalAmount)}</div>
            ...
            <div class="stat-card">
              <div class="stat-value currency">${formatCurrency(overallStats.avgServiceValue)}</div>
              ...
              <tbody>
                ${userStats.map((user: any) => `
                  <tr>
                    ...
                    <td class="currency">${formatCurrency(user.totalAmount)}</td>
                    <td class="currency">${formatCurrency(user.authorizedAmount)}</td>
                    <td class="pending">${formatCurrency(user.pendingAmount)}</td>
                  </tr>
                `).join('')}
              </tbody>
            </table>
            ...
            <tbody>
              ${monthlyStats.map(month => `
                <tr>
                  ...
                  <td class="currency">${formatCurrency(month.authorizedAmount)}</td>
                  <td class="pending">${formatCurrency(month.pendingAmount)}</td>
                  <td class="currency">${formatCurrency(month.avgServiceValue)}</td>
                </tr>
              `).join('')}
            </tbody>
          </table>
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-8">
        <div className="text-gray-600 dark:text-gray-400">Carregando análises...</div>
      </div>
    );
  }
      ...
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="flex flex-col gap-4">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-800 dark:text-white">
            {isAdmin ? 'Análises de Serviços (Admin - Todos os Serviços)' : 'Análises de Serviços'}
          </h2>
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button
              ...
              className={`${getButtonStyles('pdf')} w-full sm:w-auto justify-center`}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          <div className="bg-white dark:bg-gray-800 p-3 sm:p-6 rounded-lg shadow border border-gray-200 dark:border-gray-700">
            <div className="text-lg sm:text-2xl font-bold text-blue-600 dark:text-blue-400">{overallStats.totalServices}</div>
            <div className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Total de Serviços</div>
          </div>
          <div className="bg-white dark:bg-gray-800 p-3 sm:p-6 rounded-lg shadow border border-gray-200 dark:border-gray-700">
            ...
            <div className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Valor Total</div>
          </div>
          <div className="bg-white dark:bg-gray-800 p-3 sm:p-6 rounded-lg shadow border border-gray-200 dark:border-gray-700">
            <div className="text-lg sm:text-2xl font-bold text-orange-600 dark:text-orange-400">{overallStats.pendingServices}</div>
            <div className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Serviços Pendentes</div>
          </div>
          <div className="bg-white dark:bg-gray-800 p-3 sm:p-6 rounded-lg shadow border border-gray-200 dark:border-gray-700">
            ...
            <div className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Valor Médio por Serviço</div>
          </div>
        </div>
        ...
        {isAdmin && (
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700">
            <div className="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700">
              <h3 className="text-base sm:text-lg font-semibold text-gray-800 dark:text-white">Estatísticas por Usuário</h3>
            </div>
                ...
                <div key={userStat.username} className="p-4 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div>
                      ...
                      <span className="font-medium">{userStat.serviceCount}</span>
                    </div>
                    <div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700">
          <div className="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700">
            <h3 className="text-base sm:text-lg font-semibold text-gray-800 dark:text-white">Estatísticas Mensais</h3>
          </div>

          {/* Mobile Card View */}
          <div className="block sm:hidden divide-y divide-gray-200 dark:divide-gray-700">
            {monthlyStats.map((month) => (
              <div key={month.month} className="p-4 space-y-3">
                <div className="font-medium text-gray-900 dark:text-white text-sm">
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div>
                    <span className="text-gray-500 dark:text-gray-400">Serviços: </span>
                    <span className="font-medium">{month.serviceCount}</span>
                  </div>
                  <div>
                    <span className="text-gray-500 dark:text-gray-400">Total: </span>
                    <span className="font-medium text-green-600 dark:text-green-400">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700">
          <div className="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700">
            <h3 className="text-base sm:text-lg font-semibold text-gray-800 dark:text-white">Estatísticas por Tipo de Serviço</h3>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4">
          <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 p-3 sm:p-4 rounded-lg">
            <div className="text-sm sm:text-lg font-semibold text-green-800 dark:text-green-300">Serviços Autorizados</div>
            <div className="text-xl sm:text-2xl font-bold text-green-600 dark:text-green-400">{overallStats.authorizedServices}</div>
            <div className="text-xs sm:text-sm text-green-600 dark:text-green-400 break-all">{formatCurrency(overallStats.authorizedAmount)}</div>
          </div>
          <div className="bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 p-3 sm:p-4 rounded-lg">
            <div className="text-sm sm:text-lg font-semibold text-orange-800 dark:text-orange-300">Serviços Pendentes</div>
            <div className="text-xl sm:text-2xl font-bold text-orange-600 dark:text-orange-400">{overallStats.pendingServices}</div>
            <div className="text-xs sm:text-sm text-orange-600 dark:text-orange-400 break-all">{formatCurrency(overallStats.pendingAmount)}</div>
          </div>
          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 p-3 sm:p-4 rounded-lg">
            <div className="text-sm sm:text-lg font-semibold text-blue-800 dark:text-blue-300">Taxa de Aprovação</div>
            <div className="text-xl sm:text-2xl font-bold text-blue-600 dark:text-blue-400">
              {overallStats.totalServices > 0
                ? ((overallStats.authorizedServices / overallStats.totalServices) * 100).toFixed(1)
                : 0}%
            </div>
            <div className="text-xs sm:text-sm text-blue-600 dark:text-blue-400">
              {overallStats.authorizedServices} de {overallStats.totalServices} serviços
            </div>
          </div>
        </div>
      </div>
